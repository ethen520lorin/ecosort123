export function friendlyFirebaseError(message: string): string {
  ...
  if (value.includes('API KEY')) {
    return 'Firebase is not configured yet. Add your project values to .env.';
  }
...
export function permissionErrorMessage(kind: 'camera' | 'location' | 'notification'): string {
  ...
  if (kind === 'location') {
    return 'Location access is unavailable. EcoSort will use the cached council rules instead.';
  }
